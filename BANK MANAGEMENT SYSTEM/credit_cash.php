...
$query4_row=mysql_fetch_assoc($query4_data);
		...
		if($query1_data=mysql_query($query1) && $query2_data=mysql_query($query2) ){
			...
			echo '<br>'.'<span style="color:#0F0691;"><h2>Successful</h2>';//.$query1_data;
			echo '<h4>Account Number :</h4>'.$acc_no.'<h4>Amount credited:</h4>'.$amount;
			}
		
		else { echo '<h2>Not Done</h2>';}}
			}

		else { die('<h2>amount entered is not valid</h2>');
			}
		}
		else{
		echo '<form role="form" action="credit_cash.php" id="templatemo-preferences-form" method="POST" onsubmit="return validateForm()">
                <div class="row">
